...
*   **CPU Allocation:** Shared (1x vCPU burstable)
*   **RAM Allocation:** 512MB RAM (Standard Starter Tier limit)
*   **Database Requirements:** MongoDB Atlas (M0/M10 shared cluster)
*   **Storage Requirements:** Ephemeral filesystem storage. (S3/Cloudinary utilized for screenshot upload storage to bypass disk limitations).
*   **WebSocket Dependency:** **CRITICAL**. Heavy live duplex monitoring via Socket.IO for automated screen capture ingest, keystroke frequency logging, idle timers, and real-time state broadcasts.
*   **Current Monthly Cost:** ~$25/month (Backend Web Service) + Free Tier Frontend.
*   **Identified Bottlenecks:**
    *   High memory consumption under heavy socket loads (OOM restarts).
    *   Large CPU spikes during image processing (OCR screen analysis, canvas rendering).
    *   Frequent websocket disconnect storms when Render restarts or recycles the container.
...
*   **Database Requirements:** MongoDB Atlas (Shared Cluster)
*   **Storage Requirements:** Ephemeral storage. Less than 2GB transient logs/uploads.
*   **WebSocket Dependency:** None. Standard REST API patterns.
*   **Current Monthly Cost:** ~$25/month
*   **Identified Bottlenecks:**
    *   Severe cold starts on free/starter tiers after 15 minutes of inactivity (taking 45-60 seconds to bootstrap).
    *   Out of Memory (OOM) failures during bulk lead CSV parses or report generations due to the single-thread loop block in Node.
...
*   **CPU Allocation:** Shared (1x vCPU for Web Service + 1x vCPU for Worker)
*   **RAM Allocation:** 512MB RAM per container (1GB total)
*   **Database Requirements:** PostgreSQL (Render Managed DB) + Redis Cloud (Shared)
*   **Storage Requirements:** Ephemeral storage. Less than 1GB.
*   **WebSocket Dependency:** **High**. Ingestion and broadcast of live geographical coordinates from field delivery agents.
*   **Current Monthly Cost:** ~$57/month ($25 Web + $25 Worker + $7 Redis)
*   **Identified Bottlenecks:**
    *   Bursty CPU throttling during active route optimization cycles.
    *   Database connection starvation when Python workers fail to close pools during container re-scheduling.
    *   Render's HTTP proxy dropping long-running WebSocket tracking connections after 30 seconds of inactivity.
...
*   **CPU Allocation:** Shared (1x vCPU burstable)
*   **RAM Allocation:** 512MB RAM
*   **Database Requirements:** MongoDB Atlas (shared workspace metadata) + REST API integrations with Niyantran/CRM/Sampada DBs.
*   **Storage Requirements:** Ephemeral storage.
*   **WebSocket Dependency:** **Moderate**. Used for real-time telemetry panels showing live server statuses.
*   **Current Monthly Cost:** ~$25/month (Gateway)
*   **Identified Bottlenecks:**
    *   *Cascading Latency:* Dashboard load times are extremely slow because it aggregates REST responses from Niyantran, CRM, and Sampada—all of which are suffering from cold starts or memory limits.
    *   Timeout errors on the gateway proxy layer when downstream services take longer than 30 seconds to respond.
...
*   **CPU Allocation:** Shared (1x vCPU burstable)
*   **RAM Allocation:** 512MB RAM
*   **Database Requirements:** PostgreSQL (Managed, high-availability transaction logs)
*   **Storage Requirements:** 5GB Persistent Disk (crucial for ledger audit databases).
*   **WebSocket Dependency:** None. Strictly REST APIs for security and transaction verification.
*   **Current Monthly Cost:** ~$47/month ($25 Web Service + $7 Persistent Disk + $15 PostgreSQL Storage tier)
*   **Identified Bottlenecks:**
    *   Burstable CPU throttle: During monthly billing/invoice generations, the shared CPU gets aggressively throttled, causing calculations to stall.
    *   High I/O latency on persistent disk volumes due to Render's network-attached storage limits.
...
*   **CPU Allocation:** 2x vCPUs (shared across multiple containers)
*   **RAM Allocation:** 1GB RAM (Split: 512MB Gateway, 512MB LangGraph/Python container)
*   **Database Requirements:** MongoDB Atlas
*   **Storage Requirements:** Ephemeral storage.
*   **WebSocket Dependency:** **Moderate**. Live stream buffers from AI agents and interactive chat systems.
*   **Current Monthly Cost:** ~$82/month ($25 Frontend + $25 Gateway + $25 LangGraph + $7 Redis)
*   **Identified Bottlenecks:**
    *   **Severe Out-of-Memory (OOM) crashes:** Python-based LangGraph containers regularly exceed 512MB memory allocations when running local vector embeddings or concurrent AI pipelines.
    *   Extremely high startup latency (60s+) on cold boots due to heavy Python packages (`langchain`, `pydantic`, `spacy`) loading into memory.
...
*   **Impact on SETU:**
    *   **Niyantran:** When running OCR analyses or processing screenshots in real-time, the backend CPU spikes. The platform throttles the container, causing UI requests to hang for minutes.
    *   **Artha:** Ledger generations that should complete in milliseconds stall under throttled CPU execution.
...
*   **Impact on SETU:**
    *   **Sampada:** Python AI processes and LangGraph pipelines easily consume 300MB+ at idle. Under concurrent user chats, memory overflows the 512MB cap instantly, killing the container.
    *   **Niyantran:** In-memory image processing and high-frequency WebSocket data accumulation in the Express app cause immediate OOM crashes, resulting in unexpected mid-day downtime for employee logs.
...
*   **Impact on SETU:** Network-attached storage on Render is bandwidth-throttled. Artha’s transactional billing engine and Logistical shipment tracking experience heavy disk write queue delays (I/O wait time), leading to slow database query completions.
...
*   **Impact on SETU:** A single event-loop bottleneck in Node.js kills the whole application. If a background sync runs, all live WebSocket connections stutter, freeze, or disconnect.
...
*   **Impact on SETU:** This introduces **50-100ms** of routing overhead for every API call. In a real-time tracking application like Logistics or a live dashboard like Niyantran, this latency stacks up and ruins responsiveness.
...
*   **Impact on SETU:** Dropping connections triggers a **reconnection storm**. Thousands of desktop agent clients simultaneously attempt to reconnect, causing CPU spikes, database connection exhaustion, and cascading failures.
...
### Target Self-Hosted Specifications (Phase 2 Base)
*   **Host OS:** Debian 12 / Ubuntu 22.04 LTS (Bare-Metal or Sovereign VPS)
*   **Orchestrator:** Docker Compose v2.x (Keeping it lean, zero premature Kubernetes overhead)
*   **Resources Allocated (Dedicated):** 4x vCPUs, 8GB RAM, 80GB SSD
*   **Web Server / Proxy:** NGINX or Traefik (Handling zero-downtime service reloads and supporting persistent, long-duration WebSocket connections via custom timeout configurations).
*   **Database:** Dedicated MongoDB / PostgreSQL containers with persistent volume mounts on local storage to eliminate network-attached I/O queuing delays.
