...
const TabsTrigger = React.forwardRef(({ className, ...props }, ref) => {
  const hasCustomActive = className?.includes("data-[state=active]")
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        ...
        !hasCustomText && "text-xs sm:text-sm",
        !hasCustomPadding && "px-3 py-1.5",
        !hasCustomActive && "data-[state=active]:bg-zinc-900 data-[state=active]:text-white dark:data-[state=active]:bg-zinc-100 dark:data-[state=active]:text-zinc-950",
        className
      )}
      {...props} />
  )
