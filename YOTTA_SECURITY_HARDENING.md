...
Set owner and permissions on the `.env.production` file on the host server:
```bash
# Change owner to root/admin
sudo chown root:root /opt/setu/production/compose/.env.production

# Restrict reading/writing strictly to the owner
sudo chmod 600 /opt/setu/production/compose/.env.production
```
